...
class Solution {
  public:
    vector<int> printNearNodes(Node *root, int low, int high) {
        vector<int> ans;
        if (!root) return ans; // return empty vector if root is null

        // Explore left subtree
        if (root->data > low) {
            vector<int> left = printNearNodes(root->left, low, high);
            ans.insert(ans.end(), left.begin(), left.end()); // merge left result
        }
        ...
        if (root->data < high) {
            ...
            ans.insert(ans.end(), right.begin(), right.end()); // merge right result
        }
